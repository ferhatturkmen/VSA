...
def create_vehicle( request:VehicleBase, current_user:UserBase=Depends(get_current_user), db : Session = Depends(get_db)):
...
def get_vehicle(vehicle_id:int, current_user:UserBase=Depends(get_current_user),  db:Session=Depends(get_db)):
...
def update_vehicle(vehicle_id:int, request:VehicleQuery, current_user:UserBase=Depends(get_current_user), db:Session=Depends(get_db)):
    return vehicles_controller.update_vehicle(db, vehicle_id, request)

#delete a vehicle by id 
@router.delete("/{vehicle_id}/delete")
def delete_vehicle(vehicle_id:int, current_user:UserBase=Depends(get_current_user), db:Session=Depends(get_db)):
