...
def create_payment(request:PaymentBase, current_user:UserBase=Depends(get_current_user),  db : Session = Depends(get_db)):
...
def update_payment(payment_id:int, request:PaymentBase, current_user:UserBase=Depends(get_current_user),  db:Session=Depends(get_db)):
    return payments_controller.update_payment(db, payment_id, request)

#delete a payment by id 
@router.delete("/{payment_id}/delete")
def delete_payment(payment_id:int, current_user:UserBase=Depends(get_current_user),  db:Session=Depends(get_db)):
