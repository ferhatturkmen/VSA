...
def get_review(db:Session, review_id:int):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Exception occured while getting review"
        )
...
def get_all_review(db: Session, query_params: Optional[ReviewQuery]):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Exception occured while getting"
        )
...
def update_review(db:Session, review_id:int, request:ReviewQuery):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Exception occured while updating review"
        )
...
def delete_review(db:Session, review_id:int):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Exception occured while deleting review"
        )
